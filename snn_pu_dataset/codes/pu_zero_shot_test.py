...
# =====================================================================
# 1. AUTOMATED GENOME LOADER (Pulls latest weights from Drive)
# =====================================================================
HIDDEN_FEATURES = 32


# Automatically slice the array into our hardware parameters
...
classes = [
    ...
    {"code": "KI01", "is_fault": 1, "name": "Inner Race Fault"},
    {"code": "KA01", "is_fault": 1, "name": "Outer Race Fault"}
]
...
print("\n" + "="*70)
print("⚙️ INITIATING MASSIVE 240-FILE FORENSIC BENCHMARK")
print("="*70)
...
for cond in conditions:
    ...
    for cls in classes:
        ...
        if files_found > 0:
            ...
            print(f"   [{cls['code']} - {cls['name']}] Macro Acc: {macro_acc:6.2f}% | Micro Acc: {micro_acc:6.2f}% | TP:{macro_TP:4} | TN:{macro_TN:4} | FP:{macro_FP:4} | FN:{macro_FN:4} | (Files: {files_found}/20)")
        else:
            print(f"   [{cls['code']} - {cls['name']}] ❌ NO FILES FOUND in {class_dir_path}")
...
(TP_in, TN_in, FP_in, FN_in), (m_TP_in, m_TN_in, m_FP_in, m_FN_in) = sum_metrics(all_conds, ["KI01"])
(TP_out, TN_out, FP_out, FN_out), (m_TP_out, m_TN_out, m_FP_out, m_FN_out) = sum_metrics(all_conds, ["KA01"])
(TP_h, TN_h, FP_h, FN_h), (m_TP_h, m_TN_h, m_FP_h, m_FN_h) = sum_metrics(all_conds, ["K001"])

print("="*70)
print("🏆 GRAND AGGREGATE REPORT (MULTI-DOMAIN - ALL 240 FILES)")
print("="*70)
...
print(f"6. TOTAL HEALTHY MACRO SPECIFICITY : {calc_acc(TP_h, TN_h, FP_h, FN_h):6.2f}% (TN:{TN_h} FP:{FP_h})  <-- False Alarms")
...
(t,n,p,f), (mt,mn,mp,mf) = sum_metrics(["N15_M07_F10"]); print(f"7. 1500 RPM (Seen) MACRO           : {calc_acc(t,n,p,f):6.2f}%")
print(f"   1500 RPM (Seen) MICRO           : {calc_acc(mt,mn,mp,mf):6.2f}%")
(t,n,p,f), (mt,mn,mp,mf) = sum_metrics(["N09_M07_F10"]); print(f"8. 900 RPM (Seen) MACRO            : {calc_acc(t,n,p,f):6.2f}%")
print(f"   900 RPM (Seen) MICRO            : {calc_acc(mt,mn,mp,mf):6.2f}%")
(t,n,p,f), (mt,mn,mp,mf) = sum_metrics(["N15_M01_F10"]); print(f"9. 0.1 Nm Torque (Unseen) MACRO    : {calc_acc(t,n,p,f):6.2f}%")
print(f"   0.1 Nm Torque (Unseen) MICRO    : {calc_acc(mt,mn,mp,mf):6.2f}%")
(t,n,p,f), (mt,mn,mp,mf) = sum_metrics(["N15_M07_F04"]); print(f"10. 400 N Force (Unseen) MACRO     : {calc_acc(t,n,p,f):6.2f}%")
