...
-- Architecture: 1-32-1 Heterogeneous LIF Network (Hardware Quantized)
-- Debouncer: 12-Window Macro-Relay (Trips at >= 6 Micro-Alarms)
----------------------------------------------------------------------------------
library IEEE;
...
-- Temporal SNN Counters (Updated for 2048-sample window)
    signal step_count   : integer range 0 to 2047 := 0;
    signal macro_count  : integer range 0 to 11 := 0;
    ...
    signal macro_alarms     : integer range 0 to 12 := 0;
    ...
    begin
        if rising_edge(clk) then
            if rst = '1' then
                ...
                -- 2. Process Layer 1 (Heterogeneous Hidden Neurons)
                spk1 := (others => '0');
                for i in 0 to HIDDEN_SIZE-1 loop
                    cur1 := resize(adc_val * FC1_WEIGHTS(i), 32);
                    
                    -- ? THE HETEROGENEOUS FIX: Each neuron uses its own leak shift!
                    mem1_leaked := shift_right(mem1(i), L1_LEAK_SHIFTS(i));
                ...
                -- 4. Temporal Window Management (2048 Samples)
                if step_count = 2047 then
                    step_count <= 0;
                    mem1 <= (others => (others => '0'));
                    mem2 <= (others => '0');
                    micro_alarm_flag <= '0'; 
                    ...
                    -- 5. The Hardware Debouncer (12 Windows * 16ms = 192ms)
                    if macro_count = 11 then
                        macro_count <= 0;
                        
                        -- MASTER TRIP LOGIC (Trips if >= 6 alarms)
                        if (macro_alarms + current_window_alarm) >= 6 then
                            master_alarm <= '1'; 
                        else
                            master_alarm <= '0'; 
                        end if;
